...
sub main()
    ...
    '     tree.rbdeletei(tree.m_root.m_key)
    '     tree.printbtree
    ' loop
    tree.rbdelete(1)
    tree.printbtree
...
class trbtree
    ...
    public function rbfixup(byval node)
    ...
    public function rbdeletemin(byval node)
        if node.m_left is nothing then
            ...
            set node.m_left = rbdeletemin(node.m_left)
            set node = rbbalance(node)
        end if
        set rbdeletemin = node
    end function
